...
<form id="kt_modal_user_form" method="POST" class="form"
    action="{{ $record->id > 0 ? route('users.update', $record->id) : route('users.store') }}"
    ...
    <div class="d-flex flex-column scroll-y px-5 px-lg-10" id="kt_modal_add_user_scroll" data-kt-scroll="true"
        ...
        <x-forms.input-group.index required="1" label="Tipo" :value="$record->roles?->first()?->id"
            :options="$record->getRoles()" name="roles" variant="select2" :empty="true" :disabled="$disabled" />

        <x-forms.input-group.index variant="checkbox" label="Habilitado"
        value="{{ $record->is_enabled }}" name="is_enabled" :disabled="$disabled" />
