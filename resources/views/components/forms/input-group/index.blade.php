...
<div class="fv-row mb-7">
    ...
    @if ($variant == 'input')
        <x-forms.input.index :$type :$name :$id :$value :$readonly :$disabled :$offcomplete :$class :$placeholder :$contentCallback/>
    @endif

    @if ($variant == 'textarea')
        <x-forms.textarea.index :$type :$name :$id :$value :$readonly :$disabled :$offcomplete :$class :$placeholder :$contentCallback/>
    @endif

    @if ($variant == 'select2')
        <x-forms.select2.index :$limit :$idChild :$searchable :$options :$url :$type :$name :$id :$value :$readonly :$disabled :$contentCallback
            :$class :$idParent :$multiple :$placeholder :$keysValues :$empty/>
    @endif

    @if ($variant == 'checkbox')
        <x-forms.checkbox.index :$name :$label :$id :$value :$readonly :$disabled :$checked :$class :$placeholder :$contentCallback/>
    @endif
    ...
    @if ($variant == 'repeater')
        <x-forms.repeater.repeater :$value :$body :$id :$labelDelete :$labelAdd :$readonly :$disabled :$placeholder :$contentCallback/>
    @endif

    @if ($variant == 'select')
        <x-forms.select.index :$options :$name :$id :$value :$readonly :$disabled :$class :$placeholder :$contentCallback/>
    @endif
